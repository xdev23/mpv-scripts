...
if use_script then
	...
	-- Tell the built-in OSC that our custom thumbnailer is taking over
	...
	-- Peek into the built-in OSC configuration to see what layout is set in osc.conf
	local osc_opts = { layout = "bottombar" }
	...
	local dynamic_preview_y_offset = -75      -- Used for other layouts (slimbox), or when use_fixed_preview_height is false
	...
	local function shutdown()
		...
		mp.set_property_native("user-data/thumbnailer/enabled", false)
	end
	...
	local function on_hover_time_change(name, value)
		-- Extract hover_sec and the Y coordinate from the property
		local time_in_seconds = nil
		local thumb_y = nil
		if type(value) == "table" then
			time_in_seconds = tonumber(value.hover_sec)
			thumb_y = tonumber(value.y)
		end
		
		-- If it's NOT a number (meaning the table cleared or we dragged away), hide it immediately
		...
		if mouse and osd then
			...
			-- Track X perfectly via the mouse
			...
			-- We'll use the property's Y to determine if it's in the top half (fallback to mouse.y if missing)
			local reference_y = thumb_y or mouse.y
			is_topbar = reference_y < (osd.h / 2)

			-- Use the layout from osc.conf, but check command-line script-opts as a fallback
			local osc_layout = osc_opts.layout:lower()
			...
			if apply_fixed then
				-- Standard Thick Bars (Top/Bottom) - Use the fixed screen-edge offsets
				if is_topbar then
					preview_y = offset
				else
				...
				-- Slimbox/Dynamic - Use the property's Y coordinate to anchor perfectly to the timeline
				if is_topbar then
					preview_y = reference_y + offset + bw
				else
					preview_y = reference_y - effective_h - offset - bw
				end
			end
	...
	-- Observe the property as a "native" table instead of "string"
	mp.observe_property("user-data/osc/thumbnailer", "native", on_hover_time_change)
